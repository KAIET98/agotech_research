...
'''
1) Así que primero habrá que hacer una call a la base de datos y sacar la 
...
def insert(event, context):
  ...
  #request_json = request.get_json()
    ...
    q = 'select * from METEREOLOGIA;'



    sentencia = q

    df = run_query(sentencia)

    last_day = df.iloc[(df.shape[0]-1),0]
    ...
    '''
    2) Luego tendremos que calcular cual es el día de hoy 
    ...
    data = mi_metereologia(8175, last_day, today, API_KEY)
